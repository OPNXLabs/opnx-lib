...
namespace OPNX.Lib.Streaming.RTSP
{
    public class AuthenticationBasic : Authentication
    {
        ...
        public override bool IsValid(RtspRequest receivedMessage)
        {
            ...
            if (parts.Length == 2)
            {
                return string.Equals(parts[0], Credentials.UserName, StringComparison.OrdinalIgnoreCase)
                    && string.Equals(parts[1], Credentials.Password, StringComparison.Ordinal);
            }
            return false;
        }
