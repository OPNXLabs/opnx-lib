...
namespace OPNX.Lib.Streaming.RTSP.RTP
{
    ...
    */
    ...
    public class AACPayload : IPayloadProcessor
    {
        ...
        public uint ObjectType { get; set; } = 0;
        public uint FrequencyIndex { get; set; } = 0;
        public uint SamplingFrequency { get; set; } = 0;
        public uint ChannelConfiguration { get; set; } = 0;
        ...
        public RawMediaFrame ProcessPacket(RTPPacket packet)
        {
            ...
            var rtpPayloadSpan = rtpPayload.Span;

            // 2 bytes for AU Header Length, 2 bytes of AU Header payload
            while (position + 4 <= packet.PayloadSize)
            {
                // Get Size of the AU Header                
                int au_headers_length_bits = BinaryPrimitives.ReadUInt16BigEndian(rtpPayloadSpan[position..]);
                int au_headers_length = (int)Math.Ceiling(au_headers_length_bits / 8.0);
                position += 2;

                // Examine the AU Header. Get the size of the AAC data
                ushort au_header = BinaryPrimitives.ReadUInt16BigEndian(rtpPayloadSpan[position..]);
                int aac_frame_size = (au_header >> 3) & 0x1FFF;  // 상위 13 bits
                //int aac_index_delta = au_header & 0x07;          // 하위 3 bits
                ...
                rtpPayload.Slice(position, aac_frame_size).CopyTo(data);
                owners.Add(owner);
                audioData.Add(data);
